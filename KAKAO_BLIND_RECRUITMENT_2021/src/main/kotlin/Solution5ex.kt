...
class Solution5ex {
    ...
    fun bfs(start: Point, end: Point): Int {
        ...
        while (q.isNotEmpty()) {
            ...
            for (i in 0..3) {
                ...
                if (!visited[nx][ny]) {
                    visited[nx][ny] = true
                    q.add(Point(nx, ny, now.cnt + 1))
                }
                for (j in 0..1) {
                    if (board[nx][ny] != 0) break
                    if (nx + dx[i] < 0 || nx + dx[i] > 3 ||
                        ny + dy[i] < 0 || ny + dy[i] > 3) break
                    nx += dx[i]
                    ny += dy[i]
                }
                if (!visited[nx][ny]) {
    ...
    fun permutate(point: Point): Int {
        var ret = 1e9.toInt()
        for (num in 1..6) {
            val cards = mutableListOf<Point>()
            for (i in 0..3) {
                for (j in 0..3) {
                    if (board[i][j] == num) {
                        cards.add(Point(i, j, 0))
                    }
                }
            }
            if (cards.isEmpty()) continue
            val one = bfs(point, cards[0]) + bfs(cards[0], cards[1]) + 2
            val two = bfs(point, cards[1]) + bfs(cards[1], cards[0]) + 2

            cards.forEach {
                board[it.r][it.c] = 0
            }

            ret = Math.min(ret, one + permutate(cards[1]))
            ret = Math.min(ret, two + permutate(cards[0]))
...
fun main() {
    val result = Solution5ex().solution(arrayOf(intArrayOf(1, 0, 0, 3), intArrayOf(2, 0, 0, 0), intArrayOf(0, 0, 0, 2), intArrayOf(
        3, 0, 1, 0)), 1, 0)
    println(result)
}
